  ...
  if ( cmd.m_pTo == this )
  {
    if ( HasProperty("deadly") && GetProperty("deadly").As<bool>() )
    {
      // Report death message attached to room
		*g_Game.page << GetProperty("deathmessage").As<string>() << "\n";
		g_Game.SetProperty("running", false);
    }
    else 
    {
		*g_Game.page << "\n" << GetName() << "\n";
		*g_Game.page << GetEnterMessage() << "\n";
	...
	if ( !item->HasProperty("hidden") || item->GetProperty("hidden").As<bool>() == false )
      ...
      if ( items.empty()) 
      {
		  *g_Game.page << "There seems to be nothing in the room.";
      }
      else 
      {
		  *g_Game.page << "You can see " << Game::MakeReadable(items) << " in here.";
      }
    }
  }
  else 
  {
	  *g_Game.page << "You leave " << GetName() << ".";
    if ( cmd.m_Dir == South && HasProperty("south_exit_message") )
		*g_Game.page << GetProperty("south_exit_message").As<string>() << ".";
    if ( cmd.m_Dir == West && HasProperty("west_exit_message") )
		*g_Game.page << GetProperty("west_exit_message").As<string>() << ".";
    if ( cmd.m_Dir == North && HasProperty("north_exit_message") )
		*g_Game.page << GetProperty("north_exit_message").As<string>() << ".";
    if ( cmd.m_Dir == East && HasProperty("east_exit_message") )
		*g_Game.page << GetProperty("east_exit_message").As<string>() << ".";
  }
  *g_Game.page << "\n";
}
...
Room::Execute( TakeCommand & cmd )
{
  ...
  if ( item ) 
  {
	...
	*g_Game.page << "You picked up " << item->GetName() << "\n";
  ...
  {
	  *g_Game.page << "Can you be more specific?\n";
  }
  else 
  {
	  *g_Game.page << "There is no such item.\n";
  }
}
////////////////////////////////////////////////////////////////////////////////
void
Room::Execute( DropCommand & cmd)
{
  ...
  if ( item )
  {
	...
	*g_Game.page << "You dropped " << item->GetName() << "\n";
  }
  else 
  {
	  *g_Game.page << "You do not have such item.\n";
  }
}
////////////////////////////////////////////////////////////////////////////////
void
...
Room::Execute( LookCommand & cmd )
{
	...
	*g_Game.page << "From here you can go ";
  ...
  *g_Game.page << Game::MakeReadable(directions) << ".\n";
}
////////////////////////////////////////////////////////////////////////////////
void
Room::Execute( ExamineCommand & cmd )
{
  ...
  if ( !cmd.HasTarget() )
  { 
	  *g_Game.page << "Examine what? ";
  }
  else if ( cmd.m_strTarget == "room" )
  {
	  *g_Game.page << room.GetDescription() << "\n";
    list<string> items;
  ...
  else  
  {
	...
	if (item) *g_Game.page << "It appears to be " << item->GetDescription() << ". ";
	else     *g_Game.page << "I do not know how to examine " << cmd.m_strTarget << ". ";
  }
  *g_Game.page << "\n";
}
////////////////////////////////////////////////////////////////////////////////
...
{
	*g_Game.page << "You are not making any sense.\n";
}
////////////////////////////////////////////////////////////////////////////////
void
